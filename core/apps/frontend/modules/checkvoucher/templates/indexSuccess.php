...
<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="javascript:initAddCheckVoucher();" class="buttons"><?php echo image_tag("icons/add.png");?>New Check Voucher</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("checkvoucher/lists");?>',
    datatype: 'json',
    mtype: 'GET',
...
{
	$.ajax({
			url: '<?php echo url_for("checkvoucher/initCreateCheckVoucher");?>',
		    type: 'json', 
			method: 'POST',
			success: function(resp){
				...
				if(_resp.has_data){ 
					...
					$('.init_create_voucher_popup .init_add_check_voucher').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'btnBack'},  function() {
						$(".init_add_check_voucher").css("height","248px;");						
						$(".datepicker_s").datepicker({dateFormat: "MM d, yy"});
						$(".btnPrint").click(function(){
							$("form#form_create_check_voucher").submit();
						});
					});	
				}else{
