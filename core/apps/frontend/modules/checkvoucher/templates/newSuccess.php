...
<?php end_slot();?> 
    <div class="formBodyTransaction">
		<form action="<?php echo url_for("checkvoucher/update?id=").$form->getObject()->getId();?>"  method="post" id="form_check_voucher">
				<?php echo $form->renderHiddenFields();?>
        	<h4>Create New Receipt</h4> 
                <div class="formControlBtnTop" align="right">
                    <a href="<?php echo url_for("checkvoucher/index");?>" class="btnBack">Back</a> 
					<a href="javascript:document.getElementById('form_check_voucher').reset();" class="btnCancel">Reset</a>
            	...
            	<div class="formTransaction">
                    ...
                    <div class="transBodyRight">  
                        ...
                        <div class="transBodyContent">
                            ...
                            <div class="transBodyInfo">
                                <?php echo $form['date_received']->render();?>
                            </div>  
                        </div> 
                        <div class="transBodyContent">
                            <div class="transBodyTitle">
                        ...
                        <div class="transBodyContent">
                            ...
                            <div class="transBodyInfo">
                                <?php echo $form['si_dr_number']->render();?>
                            </div>
                        </div> 
						<div class="transBodyContent">
                            <div class="transBodyTitle">
                            ...
                            <div class="transBodyInfo" align="right">
                            	<a href="javascript:void(0);" class="btnEntry check_voucher_entry">Check Entry</a>   
                            </div>  
                        </div> 
                    </div>
			...
			<div class="entryForm"> 
                    ...
                    <div class="particularHeader">
						...
						<div class="parent_part">
							...
							<?php if(count($form['CheckVoucherParticularEntry']) > 0):?>
								<?php $total_part = 0;?>
								<?php foreach($form['CheckVoucherParticularEntry'] as $p_entry_form):?>
								<?php $total_part++;?>
			...
			<div class="entryForm"> 
					...
					<div class="particularHeader">   
						<div class="account_part"> 
							...
							<?php if(count($form['CheckVoucherAccountEntry']) > 0 ):?>
								<?php $total_acct = 0;?>
								<?php foreach($form['CheckVoucherAccountEntry'] as $a_enrty_form):?>
									<?php $total_acct ++;?>
							...
							<?php else:?> 
									...
									<?php foreach($form['account_entries'] as $a_enrty_form):?>
										...
										<div class="entryBorder account_entry_row" id="row-<?php echo $total_acct;?>">
											<div class="entryChartOR"> 
												<div> 
												<?php echo $a_enrty_form['chart_of_account_id']->render(array('class'=>'entrySelect checkvoucher-account-entry-coa'));?> 
												</div> 
											</div>                     
											<div class="entryChart">  
													<?php echo $a_enrty_form['general_library_id']->render(array('class'=>'entrySelect'));?>  
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#<?php echo $form['date_received']->renderId();?>").datepicker({dateFormat: "yy/mm/dd"});
		...
		$(".btnSave").click(function(){
			$("form#form_check_voucher").submit();
		}); 
		
		var validator = $("form#form_check_voucher").validate({		 
			rules: {		
				'<?php echo $form['voucher_number']->renderName();?>': {
						  required: true, 
						  remote: {
								url: "<?php echo url_for("checkvoucher/checkvouchernumber");?>",
								type: "post",
								data: {
			...
			messages: {
				'<?php echo $form['voucher_number']->renderName();?>': {required:"*Required",remote:'*Check Voucher already exist.'}, 
				'<?php echo $form['general_library_id']->renderName();?>': "*Required", 
				'<?php echo $form['currency_id']->renderName();?>': "*Required", 
				'<?php echo $form['total_amount']->renderName();?>': "*Required"
			},
		...
		$("#<?php echo $form['particular_template']->renderId();?>").change(function(){
				$.ajax({
					url: '<?php echo url_for("checkvoucher/loadParticularTemplate?pid=");?>'+$(this).val(),
					type: 'json',
					method: 'POST',
		...
		$(".account_part").delegate(".checkvoucher-account-entry-coa",'change',function(e){ 
			checkAccountType($(this).val());
		}); 
		
			$(".check_voucher_entry").click(function(){ 
				$.ajax({
					url: '<?php echo url_for("checkvoucher/checkentries?cv_id=".$form->getObject()->getId());?>',
					type: 'POST',
					dataType: 'json', 
					success: function(resp){ 
						...
						if(resp_data.is_valid){  
							...
							$('.checkentry_popup .popupCheck').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'closeCheckEntry',
								onClose: function() {
									$(".popupCheck").remove();
	...
	{
		$.ajax({
			url: '<?php echo url_for("checkvoucher/checkAccountType");?>',
		    type: 'json',
	...
	function calculate(obj,_type)
	{
		...
		$.ajax({ url: '<?php echo url_for('checkvoucher/calcTotalPartEntry');?>',
	...
	{
			$.ajax({
					url: '<?php echo url_for("checkvoucher/addParticularEntry?num=");?>'+ (new_particular_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
	...
	{
			$.ajax({
					url: '<?php echo url_for("checkvoucher/addAccountEntry?num=");?>'+ (new_account_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
