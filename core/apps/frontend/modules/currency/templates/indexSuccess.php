...
<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="<?php echo url_for("currency/new");?>" class="buttons"><?php echo image_tag("icons/add.png");?>Add Tax Final Withheld</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("currency/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['ID','Symbol','Title','Rate','date','Created At','Updated At','Action'],
    colModel :[  
      {name:'id', index:'id', width:30 },   
      {name:'symbol', index:'symbol', width:25, align: "center" },   
      {name:'title', index:'title', width:55 },   
      {name:'unit_per_base', index:'unit_per_base', width:55 },    
      {name:'date', index:'date', width:70, align:'center', stype: "range",
				searchoptions: {
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('currency/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('currency/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("currency/export?");?>');  
