<?php slot("content_title");?>
	Petty Cash Voucher Lists
<?php end_slot();?>

<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="javascript:iniAddPettyCash();" class="buttons"><?php echo image_tag("icons/add.png");?>New Petty Cash Voucher</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("pettycashvoucher/lists");?>',
    datatype: 'json',
    mtype: 'GET',
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('pettycashvoucher/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('pettycashvoucher/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("pettycashvoucher/export?");?>');  
...
{
	$.ajax({
			url: '<?php echo url_for("pettycashvoucher/initCreatepettycashvoucher");?>',
		    type: 'json', 
			method: 'POST',
			success: function(resp){
				...
				if(_resp.has_data){ 
					...
					$('.init_create_voucher_popup .init_add_pc_voucher').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'btnBack'},  function() {
						$(".init_add_pc_voucher").css("height","248px;");						
						$(".datepicker_s").datepicker({dateFormat: "MM d, yy"});
						$(".btnPrint").click(function(){
							$("form#form_create_pc_voucher").submit();
						});
					});	
				}else{
