...
<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="<?php echo url_for("booktemplate/new");?>" class="buttons"><?php echo image_tag("icons/add.png");?>Add Template</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("booktemplate/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['Id','Code','Name','Debit','Credit','Project','Department','Created At','Updated At','Action'],
    colModel :[  
      {name:'id', index:'id', width:30 }, 
      {name:'code', index:'code', width:55 }, 
      {name:'name', index:'name', width:150 },   
      {name:'debit', index:'debit', width:60, align: 'right' },   
      {name:'credit', index:'credit', width:60, align: 'right' },   
	  {name:'project_id', index:'project_id', width:90, align:'left',stype: 'select', searchoptions: {dataUrl: "<?php echo url_for("project/getlists");?>"}},   
	  {name:'department_id', index:'department_id', width:90, align:'left',stype: 'select', searchoptions: {dataUrl: "<?php echo url_for("department/getlists");?>"}},   
	  {name:'created_at', index:'created_at', width:90, align:'center', stype: "range",
				searchoptions: {
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('booktemplate/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('booktemplate/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("booktemplate/export?");?>');  
