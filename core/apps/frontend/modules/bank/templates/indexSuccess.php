<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="<?php echo url_for("bank/new");?>" class="buttons"><?php echo image_tag("icons/add.png");?>Add Bank</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("bank/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['Code','Name','Logo','Status','Created At','Updated At','Action'],
    colModel :[  
      {name:'code', index:'code', width:55 },  
      {name:'name', index:'name', width:180, align:'left'}, 
      {name:'logo', index:'logo', width:100, align:'left'},  
	  {name:'status', index:'status', width:80, align:'right',stype: 'select',searchoptions: { 	value:":;1:ACTIVE;2:INACTIVE"}}, 
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('bank/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('bank/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("bank/export?");?>');  
