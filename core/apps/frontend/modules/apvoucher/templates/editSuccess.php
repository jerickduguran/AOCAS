...
Accounts Payable Voucher : Edit
<?php end_slot();?> 
    <div class="formBodyTransaction">
		<form action="<?php echo url_for("apvoucher/update?id=").$form->getObject()->getId();?>"  method="post" id="form_ap_voucher">
				<?php echo $form->renderHiddenFields();?>
        	<h4>Create New Voucher</h4> 
                <div class="formControlBtnTop" align="right">
                    <a href="<?php echo url_for("apvoucher/index");?>" class="btnBack">Back</a> 
					<a href="javascript:document.getElementById('form_ap_voucher').reset();" class="btnCancel">Reset</a>
            	...
            	<div class="formTransaction">
                    ...
                    <div class="transBodyRight">  
						...
						<div class="transBodyContent">
                            ...
                            <div class="transBodyInfo">
                                <?php echo $form['purchase_order_number']->render();?>
                            </div>  
                        </div> 
						<div class="transBodyContent">
                            <div class="transBodyTitle">
			...
			<div class="entryForm"> 
                    ...
                    <div class="particularHeader">
						...
						<div class="parent_part">
							...
							<?php if(count($form['AccountsPayableVoucherParticularEntry']) > 0):?>
								<?php $total_part = 0;?>
								<?php foreach($form['AccountsPayableVoucherParticularEntry'] as $p_entry_form):?>
								<?php $total_part++;?>
			...
			<div class="entryForm"> 
					...
					<div class="particularHeader">   
						<div class="account_part"> 
							...
							<?php if(count($form['AccountsPayableVoucherAccountEntry']) > 0 ):?>
								<?php $total_acct = 0;?>
								<?php foreach($form['AccountsPayableVoucherAccountEntry'] as $a_enrty_form):?>
									<?php $total_acct ++;?>
							...
							<?php else:?> 
									...
									<?php foreach($form['account_entries'] as $a_enrty_form):?>
										...
										<div class="entryBorder account_entry_row" id="row-<?php echo $total_acct;?>">
											<div class="entryChartOR"> 
												<div> 
												<?php echo $a_enrty_form['chart_of_account_id']->render(array('class'=>'entrySelect apvoucher-account-entry-coa'));?> 
												</div> 
											</div>                     
											<div class="entryChart">  
													<?php echo $a_enrty_form['general_library_id']->render(array('class'=>'entrySelect'));?>  
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(".btnSave").click(function(){
			$("form#form_ap_voucher").submit();
		}); 
		
		var validator = $("form#form_ap_voucher").validate({		 
			rules: {		
				'<?php echo $form['voucher_number']->renderName();?>': {
						  required: true, 
						  remote: {
								url: "<?php echo url_for("apvoucher/checkvouchernumber");?>",
								type: "post",
								data: {
		...
		$("#<?php echo $form['particular_template']->renderId();?>").change(function(){
				$.ajax({
					url: '<?php echo url_for("apvoucher/loadParticularTemplate?pid=");?>'+$(this).val(),
					type: 'json',
					method: 'POST',
		...
		$(".account_part").delegate(".apvoucher-account-entry-coa",'change',function(e){ 
			checkAccountType($(this).val());
		}); 
		
			$(".ap_voucher_entry").click(function(){ 
				$.ajax({
					url: '<?php echo url_for("apvoucher/checkentries?cv_id=".$form->getObject()->getId());?>',
					type: 'POST',
					dataType: 'json', 
					success: function(resp){ 
	...
	{
		$.ajax({
			url: '<?php echo url_for("apvoucher/checkAccountType");?>',
		    type: 'json',
	...
	function calculate(obj,_type)
	{
		...
		$.ajax({ url: '<?php echo url_for('apvoucher/calcTotalPartEntry');?>',
	...
	{
			$.ajax({
					url: '<?php echo url_for("apvoucher/addParticularEntry?num=");?>'+ (new_particular_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
	...
	{
			$.ajax({
					url: '<?php echo url_for("apvoucher/addAccountEntry?num=");?>'+ (new_account_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
