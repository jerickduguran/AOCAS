<?php slot("content_title");?>
	Debit / Credit Memo Lists
<?php end_slot();?>

<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="javascript:iniAddPettyCash();" class="buttons"><?php echo image_tag("icons/add.png");?>New Memo</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("dcmemo/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['ID','Book','Period','Voucher Number','Total Amount','Currency','Status','Created At','Action'],
    colModel :[  
      {name:'id', index:'id', width:30 },   
      {name:'book_id', index:'book_id', width:30, align: "left" },   
	  {name:'period', index:'period', width:70, align:'center', stype: "range",
				searchoptions: {
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('dcmemo/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('dcmemo/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("dcmemo/export?");?>');  
	frm.find('#additional_params').html('').append(_filters);
}

function iniAddPettyCash()
{
	$.ajax({
			url: '<?php echo url_for("dcmemo/initCreatedcmemo");?>',
		    type: 'json', 
			method: 'POST',
			success: function(resp){
				var _resp =  eval("("+resp+")");				
				if(_resp.has_data){ 
					$(".init_create_memo_popup").html(_resp.data);
					$('.init_create_memo_popup .init_add_memo').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'btnBack'},  function() {
						$(".init_add_memo").css("height","248px;");						
						$(".datepicker_s").datepicker({dateFormat: "MM d, yy"});
						$(".btnPrint").click(function(){
							$("form#form_create_dcmemo").submit();
						});
					});	
				}else{
...
<div style="display:none;">
	<div class="init_create_memo_popup"></div>
</div>