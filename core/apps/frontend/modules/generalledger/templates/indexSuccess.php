<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="<?php echo url_for("generalledger/new");?>" class="buttons"><?php echo image_tag("icons/add.png");?>Add New Balance</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("generalledger/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['ID','Type','Account','Debit','credit','Created At','Updated At','Action'],
    colModel :[  
      {name:'code', index:'code', width:55 },  
      {name:'account_type', index:'account_type', width:180, align:'left'}, 
      {name:'account', index:'account', width:180, align:'left'}, 
      {name:'debit', index:'debit', width:180, align:'left'}, 
      {name:'credit', index:'credit', width:180, align:'left'}, 
    ...
    viewrecords: true,
    gridview: true,
    caption: 'General Ledger Beginning Balances',
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('generalledger/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('generalledger/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	$.each(postData, function(key, value) { 
	  _filters.append('<input type="hidden" name="'+key+'" value="'+value+'"/>');
	}); 
	
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("generalledger/export?");?>');  
