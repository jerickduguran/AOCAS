<?php slot("content_title");?>
	Tax Expanded Withheld
<?php end_slot();?>

<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="<?php echo url_for("taxfinalwithheld/new");?>" class="buttons"><?php echo image_tag("icons/add.png");?>Add Tax Final Withheld</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("taxfinalwithheld/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['Code','Nature Income Payment','Tax Rate','Created At','Updated At','Action'],
    colModel :[  
      {name:'code', index:'code', width:55 },  
      {name:'nature_income_payment', index:'nature_income_payment', width:180, align:'left'}, 
      {name:'tax_rate_percent', index:'tax_rate_percent', width:50, align:'right'}, 
    ...
    viewrecords: true,
    gridview: true,
    caption: 'taxfinalwithheld Codes',
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('taxfinalwithheld/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('taxfinalwithheld/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("taxfinalwithheld/export?");?>');  
