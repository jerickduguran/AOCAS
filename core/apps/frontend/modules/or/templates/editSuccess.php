...
<?php end_slot();?> 
    <div class="formBodyTransaction">
		<form action="<?php echo url_for("or/update?id=").$form->getObject()->getId();?>"  method="post" id="form_or">
				<?php echo $form->renderHiddenFields();?>
        	<h4>Create New Receipt</h4> 
                <div class="formControlBtnTop" align="right">
                    <a href="<?php echo url_for("or/index");?>" class="btnBack">Back</a> 
            	...
            	<div class="formTransaction">
                	...
                	<div class="transBodyLeft">
                    	...
                    	<div class="transBodyContent">
                            ...
                            <div class="transBodyInfo"> 
                                <?php echo $form['receipt_number']->render(array('readonly'=>'readonly'));?>
                            </div>
                        </div>
                    ...
                    <div class="transBodyRight">  
                        ...
                        <div class="transBodyContent">
                            ...
                            <div class="transBodyInfo">
                                <?php echo $form['mode_payment']->render();?>
                            </div>
                        </div> 
						<div class="transBodyContent">
                            <div class="transBodyTitle">
                            ...
                            <div class="transBodyInfo" align="right">
                            	<a href="javascript:void(0);" class="btnEntry cash_check_entry">Cash/Check Entry</a>   
                            </div>  
                        </div> 
                    </div>
			...
			<div class="entryForm"> 
                    ...
                    <div class="particularHeader">
						...
						<div class="parent_part">
							...
							<?php if(count($form['ReceiptParticularEntry']) > 0):?>
								<?php $total_part = 0;?>
								<?php foreach($form['ReceiptParticularEntry'] as $p_entry_form):?>
								<?php $total_part++;?>
			...
			<div class="entryForm"> 
					...
					<div class="particularHeader">   
						<div class="account_part"> 
							...
							<?php if(count($form['ReceiptAccountEntry']) > 0 ):?>
								<?php $total_acct = 0;?>
								<?php foreach($form['ReceiptAccountEntry'] as $a_enrty_form):?>
									<?php $total_acct ++;?>
							...
							<?php else:?> 
									...
									<?php foreach($form['account_entries'] as $a_enrty_form):?>
										...
										<div class="entryBorder account_entry_row" id="row-<?php echo $total_acct;?>">
											<div class="entryChartOR"> 
												<div> 
												<?php echo $a_enrty_form['chart_of_account_id']->render(array('class'=>'entrySelect or-account-entry-coa'));?> 
												</div> 
											</div>                     
											<div class="entryChart">  
													<?php echo $a_enrty_form['general_library_id']->render(array('class'=>'entrySelect'));?>  
...
<script type="text/javascript">
	...
	var reloadDatapickers = function()
	{
	...
	$(document).ready(function(){
		...
		var validator = $("form#form_or").validate({		 
			rules: {		
				'<?php echo $form['receipt_number']->renderName();?>': {
						  required: true, 
						  remote: {
								url: "<?php echo url_for("or/checkreceiptnumber");?>",
								type: "post",
								data: {
								  orig_receipt_number:  '<?php echo $form->getObject()->getReceiptNumber();?>',
								  receipt_number: function()
								  {
									return $("#<?php echo $form['receipt_number']->renderId();?>").val()
								  }
								}
							  }
						  }, 
			...
			messages: {
				'<?php echo $form['receipt_number']->renderName();?>': {required:"*Required",remote:'*Receipt already exist.'}, 
				'<?php echo $form['general_library_id']->renderName();?>': "*Required", 
				'<?php echo $form['currency_id']->renderName();?>': "*Required", 
				'<?php echo $form['total_amount']->renderName();?>': "*Required"
			},
		...
		$("#<?php echo $form['particular_template']->renderId();?>").change(function(){
				$.ajax({
					url: '<?php echo url_for("or/loadParticularTemplate?pid=");?>'+$(this).val(),
					type: 'json',
					method: 'POST',
		...
		$(".account_part").delegate(".or-account-entry-coa",'change',function(e){ 
			checkAccountType($(this).val());
		}); 
		
		$(".cash_check_entry").click(function(){
			var _type_entry = $("#<?php echo $form['mode_payment']->renderId();?>").val();
			
			$.ajax({
				url: "<?php echo url_for("or/termsofpaymententries");?>",
				type: 'POST',
				data: {"type": _type_entry,'orid': <?php echo $form->getObject()->getId();?> },
				dataType: 'json',
				success: function(resp){ 
	...
	{
		$.ajax({
			url: '<?php echo url_for("or/checkAccountType");?>',
		    type: 'json',
			data: {"account_id" : account_type,
				   'receipt_number' : $("#<?php echo $form['receipt_number']->renderId();?>").val(),
	...
	function calculate(obj,_type)
	{
		...
		$.ajax({ url: '<?php echo url_for('or/calcTotalPartEntry');?>',
	...
	{
			$.ajax({
					url: '<?php echo url_for("or/addParticularEntry?num=");?>'+ (new_particular_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
	...
	{
			$.ajax({
					url: '<?php echo url_for("or/addAccountEntry?num=");?>'+ (new_account_entry_count + 1),
					type: 'POST',
					dataType: 'json',
					success: function(resp){ 
						var response_data = eval(resp);
