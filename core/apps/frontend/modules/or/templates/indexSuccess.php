...
<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="javascript:initAddOr();" class="buttons"><?php echo image_tag("icons/add.png");?>Create New Receipt</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("or/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['ID','Book','or Number','Total Amount','Currency','Due Date','Status','Created At','Action'],
    colModel :[  
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('or/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('or/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("or/export?");?>');  
...
{
	$.ajax({
			url: '<?php echo url_for("or/initCreateOr");?>',
		    type: 'json', 
			method: 'POST',
			success: function(resp){
				var _resp =  eval("("+resp+")");				
				if(_resp.has_data){ 
					$(".init_create_or_popup").html(_resp.data);
					$('.init_create_or_popup .init_add_or').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'btnBack'},  function() {
						$(".init_add_or").css("height","248px;");						
						$(".datepicker_s").datepicker({dateFormat: "MM d, yy"});
						$(".btnPrint").click(function(){
							$("form#form_create_or").submit();
						});
					});	
				}else{
...
<div style="display:none;">
	<div class="init_create_or_popup"></div>
</div>