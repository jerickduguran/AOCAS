<?php slot("content_title");?>
	Journal Voucher Lists
<?php end_slot();?>

<?php $module = SfContext::getInstance()->getRequest()->getParameter("module",rand(1234,100000));?>
	<a href="javascript:initAddCheckVoucher();" class="buttons"><?php echo image_tag("icons/add.png");?>New Journal Voucher</a>
...
$(function(){ 
  $("#<?php echo $module;?>_list").jqGrid({
    url:'<?php echo url_for("journalvoucher/lists");?>',
    datatype: 'json',
    mtype: 'GET',
    colNames:['ID','Book','journalvoucher Number','Total Amount','Currency','Due Date','Status','Created At','Action'],
    colModel :[  
      {name:'id', index:'id', width:30 },   
      {name:'book_id', index:'book_id', width:30, align: "left" },    
      {name:'or_number', index:'or_number', width:65, align: "left" },    
	...
	gridComplete: function()
	{
		...
		{ 
			var cl = ids[i]; 
				ve = '<a title="View" href="<?php echo url_for('journalvoucher/show?id=');?>'+cl+'"><?php echo image_tag('grid_show_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
				be = '<a title="Edit" href="<?php echo url_for('journalvoucher/edit?id=');?>'+cl+'"><?php echo image_tag('grid_edit_icon.png',array('style'=>'width:15px;height:15px;'));?></a>';
...
{ 	 
	...
	var frm 	= $('form#<?php echo $module;?>_export').attr('action','<?php echo url_for("journalvoucher/export?");?>');  
	frm.find('#additional_params').html('').append(_filters);
}

function initAddCheckVoucher()
{
	$.ajax({
			url: '<?php echo url_for("journalvoucher/initCreateJournalVoucher");?>',
		    type: 'json', 
			method: 'POST',
			success: function(resp){
				...
				if(_resp.has_data){ 
					...
					$('.init_create_voucher_popup .init_add_journal_voucher').bPopup({modalColor: '#C821A1',modalClose: false,closeClass:'btnBack'},  function() {
						$(".init_add_journal_voucher").css("height","248px;");						
						$(".datepicker_s").datepicker({dateFormat: "MM d, yy"});
						$(".btnPrint").click(function(){
							$("form#form_create_journal_voucher").submit();
						});
					});	
				}else{
